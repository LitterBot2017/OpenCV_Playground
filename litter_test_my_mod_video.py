...
cv2.createTrackbar('blur_size', window_nm, 7 , 75, nothing)
...
# video_capture = cv2.VideoCapture("..\randomCVshit_vids\WIN_20161025_16_46_54_Pro.mp4")
# video_capture = cv2.VideoCapture('WIN_20161025_16_46_54_Pro.mp4')
video_capture = cv2.VideoCapture('WIN_20161025_16_54_51_Pro.mp4')
# WIN_20161025_16_54_51_Pro.mp4
# fourcc = cv2.VideoWriter_fourcc(*'FMP4')
# video_capture.set(cv2.cv.CV_CAP_PROP_FOURCC,fourcc)

print video_capture
if not video_capture.isOpened():
    print "VIDEO CAN'T BE OPENED" + repr(video_capture.get(cv2.cv.CV_CAP_PROP_FOURCC))
    exit()
...
while True:
    ...
    # if True:
        ...
        blur = cv2.GaussianBlur(imgray, (blur_size, blur_size), 0)
        ...
        if s == 1:
            ...
            for cnt in contours:
                ...
                if min_area < a < max_area:
                    ...
                    bckgrnd_bgr = np.zeros((1,1,3), np.uint8)
                    bckgrnd_bgr[:,:] = (b,g,r)
                    bckgrnd_lab = cv2.cvtColor(bckgrnd_bgr, cv2.COLOR_BGR2LAB)

                    # contColor_lab = convert_color(sRGBColor(roi_mean[2],roi_mean[1], roi_mean[0]), LabColor)
                    contColor_brg = np.zeros((1,1,3), np.uint8)
                    contColor_brg[:,:] = (roi_mean[0],roi_mean[1],roi_mean[2])
                    contColor_lab = cv2.cvtColor(contColor_brg, cv2.COLOR_BGR2LAB)

                    # delta_color = round(delta_e_cie2000(bckgrnd_lab, contColor_lab),1)
                    # No easy OpenCV delta e cie 2000, so I'm going with a sort of cie 1976
                    delta_color = round(cv2.norm(bckgrnd_lab, contColor_lab),1)

                    if delta_color >= max_delta:
                      # if is_allowed_color(int_roi_mean, avg_roi_mean, max_dev):
                      # cv2.rectangle(im, (x, y), (x+h, y+w), int_roi_mean, 2)
                      cv2.rectangle(im, (x, y), (x+h, y+w), (0,0,255), 2)
                      ttl_color += 1
